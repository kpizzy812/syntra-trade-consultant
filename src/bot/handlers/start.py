...
def get_main_menu(language: str = 'ru') -> InlineKeyboardMarkup:
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text=i18n.get('menu.help', language), callback_data="menu_help"),
            InlineKeyboardButton(text=i18n.get('menu.profile', language), callback_data="menu_profile"),
        ],
        [
            InlineKeyboardButton(text=i18n.get('menu.referral', language), callback_data="menu_referral"),
            InlineKeyboardButton(text=i18n.get('menu.premium', language), callback_data="menu_premium"),
        ],
    ])
    return keyboard
...
async def cmd_start(message: Message, session: AsyncSession, user_language: str = 'ru'):
    ...
    if is_new:
        logger.info(f"New user registered: {user.id} (@{user.username}) with language: {lang}")
