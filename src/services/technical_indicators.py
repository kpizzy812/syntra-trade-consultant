...
class TechnicalIndicators:
    ...
    def calculate_all_indicators(self, df: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            if df is None or len(df) < 20:
                logger.warning("Insufficient data for indicator calculation (need at least 20 candles)")
                return None
            ...
            try:
                ...
                indicators['macd_crossover'] = self._classify_macd(macd_value, macd_signal)
            except Exception as e:
            ...
            try:
                ema_20 = ta.trend.EMAIndicator(close=close_series, window=20).ema_indicator().iloc[-1]
                ema_50 = ta.trend.EMAIndicator(close=close_series, window=50).ema_indicator().iloc[-1]

                indicators['ema_20'] = round(ema_20, 2)
                indicators['ema_50'] = round(ema_50, 2)
                indicators['ema_trend'] = self._classify_ema_trend(close, ema_20, ema_50)
            except Exception as e:
            ...
            try:
                if len(df) >= 200:
                    sma_200 = ta.trend.SMAIndicator(close=close_series, window=200).sma_indicator().iloc[-1]
                    indicators['sma_200'] = round(sma_200, 2)
                    indicators['sma_200_position'] = "above" if close > sma_200 else "below"
                else:
            ...
            try:
                ...
                indicators['bb_position'] = self._classify_bb_position(close, bb_upper, bb_middle, bb_lower)
            except Exception as e:
            ...
            # === Stochastic Oscillator ===
            try:
                stoch = ta.momentum.StochasticOscillator(high=high, low=low, close=close_series, window=14, smooth_window=3)
            ...
            try:
                adx = ta.trend.ADXIndicator(high=high, low=low, close=close_series, window=14)
            ...
            try:
                ...
                obv = ta.volume.OnBalanceVolumeIndicator(close=close_series, volume=volume)
                ...
                indicators['volume_trend'] = "increasing" if recent_volume > previous_volume else "decreasing"
            except Exception as e:
                logger.warning(f"Error calculating Volume indicators: {e}")
                indicators['obv'] = None

            logger.info(f"Calculated {len([v for v in indicators.values() if v is not None])} indicators successfully")
            return indicators
    ...
    @staticmethod
    def _classify_bb_position(price: float, upper: float, middle: float, lower: float) -> str:
    ...
    def format_indicators_for_prompt(self, indicators: Dict[str, Any]) -> str:
        ...
        if indicators.get('rsi'):
            lines.append(f"RSI (14): {indicators['rsi']} ({indicators.get('rsi_signal', 'N/A')})")
        ...
        if indicators.get('sma_200'):
            lines.append(f"SMA 200: {indicators['sma_200']} (Price {indicators.get('sma_200_position', 'N/A')})")
        ...
        if indicators.get('stoch'):
            lines.append(f"Stochastic: {indicators['stoch']} ({indicators.get('stoch_signal', 'N/A')})")

        # ADX
        if indicators.get('adx'):
            lines.append(f"ADX: {indicators['adx']} (Trend: {indicators.get('trend_strength', 'N/A')})")
