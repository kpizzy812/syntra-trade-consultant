        ...
        try:
            ...
            if result.get("success"):
                ...
                if test["name"] == "get_crypto_price":
                    ...
                    print(
                        f"  📊 24h Change: {result.get('change_24h_percent', 0):+.2f}%"
                    )
                    ...
                    print(
                        f"  Data sources: {', '.join(result.get('data_sources', []))}"
                    )
                    ...
                    if result.get("fear_greed", {}).get("value"):
                        ...
                        print(
                            f"  Fear & Greed: {fg['value']}/100 ({fg.get('value_classification')})"
                        )
                    ...
                    for coin in coins:
                        print(
                            f"  {coin.get('symbol')}: ${coin.get('price_usd', 0):,.2f} ({coin.get('change_24h_percent', 0):+.2f}%)"
                        )
                    ...
                    for idx, coin in enumerate(coins, 1):
                        print(
                            f"  {idx}. {coin.get('name')} ({coin.get('symbol')}): ${coin.get('price_usd', 0):,.2f}"
                        )
